...
#CS Header
	...
	;                  [v1.2], 20.05.2010
	;                   + Added _Encoding_ISO8859_5To1251 function.
	;
	;                  [v1.1], 25.01.2010
...
Func _Encoding_Base64Decode($sData)
	...
	Return BinaryToString(BinaryMid(DllStructGetData($Ouput, 1), 1, $Ret[0] + 2))
EndFunc   ;==>_Encoding_Base64Decode

;Description: Encode string to Base64 data format
;Author: trancexx, Ward
Func _Encoding_Base64Encode($sData, $iLineBreak = 76)
	...
	$Data = Binary($sData)
	Local $Input = DllStructCreate("byte[" & BinaryLen($sData) & "]")
	DllStructSetData($Input, 1, $sData)

	$iLineBreak = Floor($iLineBreak / 4) * 4
	Local $OputputSize = Ceiling(BinaryLen($sData) * 4 / 3)
	$OputputSize = $OputputSize + Ceiling($OputputSize / $iLineBreak) * 2 + 4

	Local $Ouput = DllStructCreate("char[" & $OputputSize & "]")

	DllCall("user32.dll", "none", "CallWindowProc", "ptr", DllStructGetPtr($CodeBuffer), _
			"ptr", DllStructGetPtr($Input), _
			"int", BinaryLen($sData), _
			"ptr", DllStructGetPtr($Ouput), _
			"uint", $iLineBreak)
...
;Description: Converts HEX symbols in string (Ex.: "=D2=C1=C2=CF") to ANSI symbols
;Author: Latoid
...
Func _Encoding_UTF8BOMDecode($sString)
	...
	Local $sDecodeStr = 'àáâãäåæçèéêëìíîïðñòóôõö÷øùüûýþÿ'
